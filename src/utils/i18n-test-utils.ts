...
export function testTranslationKey(
  ...
  const results: Record<
    string,
    { exists: boolean; translation: string; expected: string }
  > = {}
...
export function generateTranslationReport(
  testResults: ReturnType<typeof testTranslations>
) {
  const report = {
    summary: {
 ...
 */
export function printTranslationReport(
  report: ReturnType<typeof generateTranslationReport>
) {
...
export function quickTranslationTest(key: string, logResults = true): boolean {
  ...
  const allExist = Object.values(results).every(
    (translation) => translation !== key && translation.length > 0
  )
